...
function get_registers_for_property(
	...
	switch (type) {
		...
		case 'triggerActive': return [
			{ name: `${name}.Trigger.Active`,      type: input},
			{ name: `${name}.Trigger.PlayerId`,    type: input},
			{ name: `${name}.Trigger.ForceActive`, type: inout},
		];
		case 'sign': return [
			{ name: `${name}.Sign.Write`,   type: output},
			{ name: `${name}.Sign.Select`,  type: output},
			{ name: `${name}.Sign.Control`, type: output},
			{ name: `${name}.Sign.Number`,  type: output},
		];
		case 'player': return [
			{ name: `Player.SelectIndex`, type: output},
			{ name: `Player.SelectId`,    type: output},
			{ name: `Player.SelectPart`,  type: output},
			{ name: `Player.Control`,     type: output},
			{ name: `Player.Col.Select`,  type: output},
