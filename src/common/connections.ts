...
import { add_register, RegisterType } from '@/common/registers';
import {
...
function get_registers_for_property(
	...
	switch (type) {
		...
		case 'player': return [
			...
			{ name: `Player.Vel.X`,     type: inout},
			{ name: `Player.Vel.Y`,     type: inout},
			{ name: `Player.Vel.Z`,     type: inout},
			{ name: `Player.Tel.Pos.X`, type: output},
			{ name: `Player.Tel.Pos.Y`, type: output},
			{ name: `Player.Tel.Pos.Z`, type: output},
			{ name: `Player.Tel.Rot.Y`, type: output},
			{ name: `Player.Tel.Apply`, type: output},
		];
